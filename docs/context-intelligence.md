...
1. Указать действующий Bot API token в `TELEGRAM_BOT_TOKEN`. Этот же токен обслуживает команды
   FVM и Context Intelligence; отдельного polling-клиента нет.
2. Добавить бота в нужный канал/чат с правами, позволяющими получать публикации. Для каналов это
   обычно требует назначения бота администратором; приложение само права не запрашивает и не
   повышает.
3. Запустить FVM. Единственный long-polling loop запрашивает `message`, `callback_query`,
...
4. При `MISSING_TOKEN` или `UNAUTHORIZED` Telegram UI и Telegram Context отключаются без повторного
   polling-спама, а фоновое обновление футбольных данных продолжает работать.

Bot API доставляет только будущие updates, доступные конкретному боту. Он не предоставляет
произвольную загрузку старых публикаций и не раскрывает закрытые каналы без законного членства.
