...
function goalDifference(value){const explicit=Number(value?.goalDifference);return Number.isFinite(explicit)?explicit:Number(value?.goalsFor)-Number(value?.goalsAgainst);}
...
export function buildBaselineV2H(fixture,context,p=BASELINE_V2_H_PARAMETERS){
  ...
  const ppg=Number(home.points)/hp-Number(away.points)/ap,gd=goalDifference(home)/hp-goalDifference(away)/ap;
